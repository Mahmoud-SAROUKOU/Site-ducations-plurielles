...
/**
 * API - Synchronisation locale -> Hostinger
 * Reçoit des ajouts (articles, ads, admins) depuis admin.html
 */
...
try {
    if ($type === 'article') {
        if (!tableExists('articles')) {
            throw new Exception('Table articles introuvable');
        }
        if (!tableExists('users')) {
            throw new Exception('Table users introuvable');
        }
    ...
    if ($type === 'admin') {
        ...
        echo json_encode([
            'success' => true,
            'created_in' => $created,
            'id_admins' => $adminId ?? null,
            'id_users' => $userId ?? null
        ]);
        exit;
    }
